...
set_zkrush(){
    ...
    while :; do
        ...
        [[ -z $ACCOUNT_NAME ]] && ACCOUNT_NAME="dudu1luchang"
    ...
    while :; do
        ...
        [[ -z $POOL ]] && POOL="aleo.zkrush.com:3333"
...
inst_f2_cn(){
    ...
    wget --no-check-certificate  http://download.minerhome.org/aleo/data/tools/libnvidia-ml.so    -O  $pool_path/libnvidia-ml.so
    wget --no-check-certificate  http://download.minerhome.org/aleo/data/tools/libnvidia-ml.so    -O  /usr/lib/x86_64-linux-gnu/libnvidia-ml.so
...
inst_zkrush(){
    ...
    if test "$VERSION_ID" == "20.04"; then
        wget  --no-check-certificate  https://github.com/zkrush/aleo-pool-client/releases/download/v1.2/aleo-pool-prover-ubuntu2004   -O  $pool_path/aleo-pool-prover
    fi
        
    if test "$VERSION_ID" == "18.04"; then
        wget  --no-check-certificate  https://github.com/zkrush/aleo-pool-client/releases/download/v1.2/aleo-pool-prover-ubuntu1804   -O  $pool_path/aleo-pool-prover
    fi



    wget  --no-check-certificate  http://download.minerhome.org/aleo/data/zkrush/aleo-zk.sh   -O  $pool_path/aleo-zk.sh
    # wget  --no-check-certificate  http://download.minerhome.org/aleo/data/zkrush/aleo-pool-prover   -O  $pool_path/aleo-pool-prover
...
echo "本脚本从各官方网站下载集合而成, 大家可自行比较文件,更新也可以自行从官方下载后覆盖"
echo "适合 hiveos, ubuntu18.04, 20.04, 推荐20.04"
...
echo "  7、安装驱动和cuda  inst driver and cuda"
...
6)  
    ...
    ;;    
7)
    echo "安装驱动和cuda  inst driver and cuda"
    inst_driver_cuda
    ;;    
8)
    uninstall
